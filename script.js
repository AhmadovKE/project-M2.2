...
class Deposit {
    ...
    findOffers(client) {
      ...
      const fitOffer = this.res.filter(b => {
        if (client.canDeposit) {
          if (client.deposit >= b.sumMin && client.deposit <= b.sumMax && client.canDeposit == b.canDeposit && client.period >= b.termMin && client.period <= b.termMax && client.currency.toLowerCase() == b.currency.toLowerCase()) {
            return b;
          }
        } else {
          if (client.deposit > b.sumMin && client.deposit < b.sumMax && client.period >= b.termMin && client.period <= b.termMax && client.currency.toLowerCase() == b.currency.toLowerCase()) {
            return b;
          }
        }
      });
  ...
  class Application {
    ...
    showOffers = () => {
      if (this.checkInput(+this.startDoc.value, +this.monthIncreaseDoc.value, +this.periodDoc.value, this.currency.value.toLowerCase())) {
        let client = new Deposit(+this.startDoc.value, +this.monthIncreaseDoc.value, +this.periodDoc.value, this.currency.value);
    ...
    checkInput(startDoc, monthIncreaseDoc, periodDoc, currency) {
      let mainDiv = document.querySelector('.main');
